...
export default function CreateTablePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.prefix.trim()) {
      setToast({ message: 'Enter a table name', type: 'error' });
      return;
    }
    setSaving(true);
    setToast(null);
    try {
      ...
      if (made === 0) {
        setToast({ message: 'Failed to create tables', type: 'error' });
        return;
      }
      setToast({
        message: made === 1 ? 'Table created' : `${made} tables created`,
        type: 'success',
      });
      setTimeout(() => router.push('/rms/tables'), 800);
    } catch (err: any) {
      setToast({ message: err.response?.data?.message || 'Failed to create tables', type: 'error' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const inputCls =
    'h-9 w-full rounded-md border border-gray-200 px-3 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100';
  ...
  return (
    <PermissionGuard permission="tables.create">
      <div className="w-full max-w-2xl space-y-5">
        ...
        <PageHeader
          title="Add tables"
          subtitle="Create one table or a whole batch at once"
          breadcrumbs={[
            { label: 'Restaurant', href: '/rms/tables' },
            { label: 'Tables', href: '/rms/tables' },
            { label: 'Add' },
          ]}
        />
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1.5 block text-[13px] font-medium text-gray-700 dark:text-gray-300">
                Name {form.quantity > 1 ? 'prefix' : ''} <span className="text-red-500">*</span>
              </label>
              <input
                autoFocus
                value={form.prefix}
          ...
          <div className="rounded-xl bg-gray-50 px-4 py-3 text-[13px] dark:bg-gray-800/50">
            <span className="text-gray-500 dark:text-gray-400">Will create </span>
            <span className="font-medium text-gray-900 dark:text-gray-100">
              {names.length} table{names.length === 1 ? '' : 's'}
            </span>
            <span className="text-gray-500 dark:text-gray-400">: {previewLabel}</span>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button type="submit" loading={saving} disabled={!form.branchId || !form.prefix.trim()}>
              {form.quantity > 1 ? `Create ${names.length} tables` : 'Create table'}
            </Button>
          </div>
        </form>
      </div>
