...
export default function CartPanel({
  ...
  const tableName = meta.tableId
    ? tables.find((t) => t.id === meta.tableId)?.name ||
      `Table ${tables.find((t) => t.id === meta.tableId)?.number ?? ''}`.trim()
    : '';
      ...
      <div className="border-b border-gray-100 px-4 py-3 dark:border-gray-800">
        ...
        >
          <span className="flex items-center gap-2 text-gray-600 dark:text-gray-300">
            ...
            {detailsSummary || 'Add customer, table & notes'}
          </span>
        ...
        {empty ? (
          <div className="flex h-full flex-col items-center justify-center py-10 text-center">
            ...
            <p className="mt-1 max-w-[14rem] text-xs text-gray-400 dark:text-gray-500">
              Tap a product on the left to start building this order.
            </p>
          </div>
        ) : (
      ...
      <div className="border-t border-gray-100 px-4 py-3 dark:border-gray-800">
        ...
        >
          {saving ? (
            ...
            <>
              <PaymentIcon size={20} />
              {empty ? 'Charge' : `Charge ${formatNaira(total)}`}
            </>
          )}
        </button>
      </div>
