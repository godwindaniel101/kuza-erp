...
export default function StatusBadge({
  ...
  return (
    <span
      className={`inline-flex items-center font-medium rounded-full ${sizes.wrap} ${token.bg} ${token.text} ${className}`}
    >
      <i className={`bx ${icon ?? token.icon} ${sizes.icon}`} aria-hidden="true"></i>
      <span>{text}</span>
    </span>
  );
}
