  ...
  const columns: DataTableColumn<JournalEntry>[] = [
    {
      key: 'entryNumber',
      label: 'Entry #',
      render: (e) => <span className="font-medium tabular-nums text-gray-900 dark:text-white">{e.entryNumber}</span>,
    },
    { key: 'date', label: 'Date', render: (e) => <span className="tabular-nums">{formatDate(e.date)}</span> },
    {
      key: 'memo',
      label: 'Memo',
      render: (e) => <span className="block max-w-xs truncate">{e.memo || '-'}</span>,
    },
    { key: 'lines', label: 'Lines', align: 'center', render: (e) => <span className="tabular-nums">{(e.lines || []).length}</span> },
    {
      key: 'amount',
      label: 'Amount',
      align: 'right',
      render: (e) => <span className="tabular-nums">{formatMoney(entryTotal(e), currency)}</span>,
    },
    {
      key: 'status',
      label: 'Status',
