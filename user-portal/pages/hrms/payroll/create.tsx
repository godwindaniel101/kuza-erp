      ...
      <Card>
        <form onSubmit={handleSubmit} className="w-full max-w-3xl space-y-5">
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
            <h3 className="font-display text-sm font-semibold tracking-tight text-gray-900 dark:text-gray-100 mb-4">{t('payrollItems')}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
              <input
                ...
                className="h-9 px-3 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus-visible:ring-1 focus-visible:ring-accent-ring focus-visible:border-transparent text-sm"
              />
              <input
                type="number"
                ...
                {formData.items.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <div className="flex-1 grid grid-cols-4 gap-4">
                      ...
                      <span className={`text-sm tabular-nums ${item.isEarning ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-gray-900 dark:text-gray-100 tabular-nums">${totals.grossPay.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600 dark:text-gray-400">{t('totalDeductions')}:</span>
                <span className="font-medium text-red-600 dark:text-red-400 tabular-nums">${totals.totalDeductions.toFixed(2)}</span>
              </div>
              <div className="border-t border-gray-200 dark:border-gray-700 pt-2 flex justify-between">
                <span className="font-medium text-gray-900 dark:text-gray-100">{t('netPay')}:</span>
                <span className="font-bold text-lg text-gray-900 dark:text-gray-100 tabular-nums">${totals.netPay.toFixed(2)}</span>
              </div>
            </div>
          </div>
