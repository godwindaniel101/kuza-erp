      ...
      {loading ? (
        ...
        <>
          ...
          >
            ...
            <div className="flex items-start justify-between flex-wrap gap-4 pb-6 border-b border-gray-200 dark:border-gray-700">
              ...
              <div className="text-right">
                <h2 className="font-display text-2xl font-bold tracking-tight" style={{ color: accent }}>
                  {t('invoices.invoiceUpper', 'INVOICE')}
                </h2>
                <p className="mt-0.5 text-sm tabular-nums text-gray-500 dark:text-gray-400">{invoice.invoiceNumber}</p>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800/60">
                  {invoice.lines.map((line) => (
                    <tr key={line.id}>
                      ...
                      <td className="py-3 text-sm text-right tabular-nums text-gray-700 dark:text-gray-300">
                        {formatMoney(line.unitPrice, currency)}
                      </td>
                      <td className="py-3 text-sm text-right tabular-nums text-gray-700 dark:text-gray-300">
                        {line.taxRate != null && Number(line.taxRate) !== 0 ? `${line.taxRate}%` : '-'}
                      </td>
                      <td className="py-3 text-sm text-right font-medium tabular-nums text-gray-900 dark:text-white">
            ...
            <div className="flex justify-end border-t border-gray-200 dark:border-gray-700 pt-4">
              <div className="w-full sm:w-72 text-sm space-y-2">
                ...
                <div className="flex justify-between text-gray-600 dark:text-gray-400">
                  ...
                  <span className="tabular-nums">{formatMoney(invoice.taxTotal, currency)}</span>
                </div>
                <div className="flex justify-between text-gray-600 dark:text-gray-400">
                  <span>{t('invoices.discount', 'Discount')}</span>
                  <span className="tabular-nums">-{formatMoney(invoice.discountTotal, currency)}</span>
                </div>
                ...
                <div className="flex justify-between text-gray-600 dark:text-gray-400">
                  ...
                  <span className="tabular-nums">{formatMoney(invoice.amountPaid, currency)}</span>
                </div>
                <div className="flex justify-between font-bold text-gray-900 dark:text-white text-base border-t border-gray-200 dark:border-gray-700 pt-2">
                  ...
                  <span className="tabular-nums">{formatMoney(invoice.balance, currency)}</span>
                </div>
              </div>
            </div>
          ...
          <div className="no-print max-w-4xl">
            <h2 className="font-display text-[15px] font-semibold tracking-tight text-gray-900 dark:text-gray-100 mb-3">{t('invoices.payments', 'Payments')}</h2>
