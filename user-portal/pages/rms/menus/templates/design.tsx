    ...
    <PermissionGuard permission="menus.edit">
      <div className="kz-stagger max-w-4xl mx-auto space-y-4">
        ...
        {template && menu ? (
                    ...
                    <div key={idx}>
                      ...
                      {category.items && category.items.slice(0, 3).map((item: any, itemIdx: number) => (
                        <div key={itemIdx} className="flex justify-between py-1 text-sm">
                          ...
                          <span className="font-semibold tabular-nums" style={{ color: template.theme_settings?.primaryColor || '#dc2626' }}>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card ring-1 ring-gray-950/[0.04] dark:ring-gray-800 p-8 text-center">
            <p className="text-[13px] text-gray-500 dark:text-gray-400">{t('templateNotFound') || 'Template not found'}</p>
            <Link href={`/rms/menus/templates?menu_id=${menu_id}`} className="mt-4 inline-block text-[13px] font-medium text-accent hover:text-accent-hover">
