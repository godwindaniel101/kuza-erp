...
export default function CreateOrderPage() {
  ...
  const loadInitialData = async () => {
    try {
      const [branchesRes, tablesRes] = await Promise.all([
        ...
        api
          .get<{ success: boolean; data: any[] }>("/rms/tables")
  ...
  const hasPendingItem = () => {
    return (
      selectedItem && itemQuantity > 0 && (selectedUomId || selectedItem?.uomId)
    );
  };
  ...
  const addOrderItem = () => {
    if (!branchId) {
      setToast({
        message: t("pleaseSelectBranch") || "Please select a branch first",
        type: "error",
      });
      return;
    }
  ...
  const getAvailableUomsForItem = (
    ...
    if (!item?.uoms || !item?.uomToBase) {
      ...
      if (stockInBase > 0 && item?.baseUomId) {
        return [
          { id: item.baseUomId, name: item.unit || "Unit", abbreviation: "" },
        ];
      }
      return [];
    }
    ...
    const updated = orderItems.map((item) => {
      ...
      if (field === "inventoryItemId") {
        ...
        if (selectedItem) {
          ...
          const availableUoms = getAvailableUomsForItem(
            selectedItem,
            stockInBase,
          );
          ...
          const defaultUomId =
            availableUoms.find((uom: any) => uom.id === baseUomId)?.id ||
            availableUoms[0]?.id ||
            "";
          const defaultPrice =
            selectedItem.uomPrices?.[defaultUomId] ||
            selectedItem.price ||
            selectedItem.basePrice ||
            0;
  ...
  const getPendingItemTotal = () => {
    ...
    const unitPrice =
      selectedItem?.uomPrices?.[selectedUomId || selectedItem.uomId || ""] ||
      selectedItem?.price ||
      selectedItem?.basePrice ||
      0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      itemsToSubmit.some((item) => !item.inventoryItemId || !item.uomId)
    ) {
      setToast({
        message: t("pleaseAddItems") || "Please add at least one valid item",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        setToast({
          message: t("orderCreated") || "Order created successfully",
          type: "success",
        });
          ...
          <div className="grid grid-cols-1 xl:grid-cols-10 gap-6">
            ...
            <div className="xl:col-span-7">
              <div className="flex w-full justify-between">
                <div className="mb-6">
                  <Link
                    href="/rms/orders"
                    className="text-accent hover:text-accent-hover hover:underline mb-2 inline-block transition-colors"
                  >
                    ← {t("backToOrders") || "Back to Orders"}
                  </Link>
                  <h1 className="font-display text-[1.35rem] font-semibold tracking-tight text-gray-900 dark:text-gray-100">
                ...
                <div className="mb-6 pb-6">
                  ...
                  <div className="">
                    <SearchableSelect
                      ...
                      focusColor="red"
                      searchPlaceholder={
                        t("searchBranch") || "Search branch..."
                      }
                    />
                  </div>
                  {!branchId && (
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      {t("selectBranchFirst") || "Please select a branch first"}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div
                className={`${!branchId ? "pointer-events-none opacity-50" : ""}`}
              >
                <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card ring-1 ring-gray-950/[0.04] dark:ring-gray-800 p-6">
                  ...
                  <h2 className="font-display text-[15px] font-semibold tracking-tight text-gray-900 dark:text-gray-100 mb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-4 mb-4">
                    <div>
                      <SearchableSelect
                        ...
                        onChange={(value) => {
                          ...
                          if (item) {
                            const stockInBase = Number(
                              item.stock || item.baseQuantity || 0,
                            );
                            const availableUoms = getAvailableUomsForItem(
                              item,
                              stockInBase,
                            ...
                            const defaultUomId =
                              availableUoms.find(
                                (uom: any) => uom.id === baseUomId,
                              )?.id ||
                              availableUoms[0]?.id ||
                              "";
                            const defaultPrice =
                              item.uomPrices?.[defaultUomId] ||
                              item.price ||
                              item.basePrice ||
                              0;
                    ...
                    <div>
                      <input
                        ...
                        className="h-9 w-full px-3 text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-ring focus-visible:border-transparent"
                        ...
                        selectedItem.stock !== undefined && (
                          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            {(() => {
                              ...
                              const multiplier =
                                selectedItem.uomToBase?.[selectedUomId] || 1;
                              const availableInUom = Math.floor(
                                stockInBase / multiplier,
                              );
                              const selectedUom = selectedItem.uoms?.find(
                                (u: any) => u.id === selectedUomId,
                              );
                              const uomName =
                    ...
                    <div>
                      <button
                        ...
                        className="w-full px-3 py-2 bg-accent text-accent-fg rounded-lg hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center h-9"
                                ...
                                item.uomId && (
                                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                    {(() => {
                                      ...
                                      const multiplier =
                                        item.uomToBase?.[item.uomId] || 1;
                                      const availableInUom = Math.floor(
                                        stockInBase / multiplier,
                                      );
                                      const selectedUom =
                                        item.availableUoms?.find(
                                          (u: any) => u.id === item.uomId,
                                        );
                                      const uomName =
                                        selectedUom?.abbreviation ||
                                        selectedUom?.name ||
                    ...
                    <div className="flex items-center space-x-2">
                      <label className="flex items-center cursor-pointer">
                        <input
                          ...
                          className="w-4 h-4 text-red-600 border-gray-300 rounded focus-visible:ring-accent-ring"
                        />
                        <span className="ml-2 text-xs font-medium text-gray-700 dark:text-gray-300">
                          {t("applyVat") || "Apply VAT"}
                        </span>
                      </label>
                      {formData.applyVat && (
                        <div className="flex items-center space-x-1">
                          <input
                            ...
                            max="100"
                            step="0.1"
                            className="h-9 w-16 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-ring focus-visible:border-transparent"
                          />
                ...
                {showDetails && (
                  <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card ring-1 ring-gray-950/[0.04] dark:ring-gray-800 p-6">
                    <h3 className="font-display text-[15px] font-semibold tracking-tight text-gray-900 dark:text-gray-100 mb-4 flex items-center">
                    ...
                    <div className="space-y-1">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            className="h-9 w-full px-3 text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-ring focus-visible:border-transparent"
                          >
                            <option value="">
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          rows={3}
                          className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-ring focus-visible:border-transparent resize-none"
                          placeholder={
                            t("orderNotes") ||
            ...
            <div className="xl:col-span-3 space-y-4">
              ...
              <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 shadow-card ring-1 ring-gray-950/[0.04] dark:ring-gray-800 xl:sticky xl:top-6">
                ...
                >
                  <span className="font-display text-[15px] font-semibold tracking-tight text-gray-900 dark:text-gray-100">
                    {t("orderSummary") || "Order Summary"}
                  </span>
                  <i
                    className={`bx text-xl text-gray-500 dark:text-gray-400 transition-transform ${showOrderSummary ? "bx-chevron-up" : "bx-chevron-down"}`}
                  ></i>
                </button>
                ...
                <div className="flex items-center justify-between pt-4">
                  ...
                  <span className="font-display text-2xl font-bold text-red-600 dark:text-red-400 tabular-nums">
