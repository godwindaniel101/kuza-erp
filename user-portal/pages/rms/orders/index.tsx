  ...
  const columns: DataTableColumn<SaleRow>[] = [
    {
      ...
      label: t('orderNumber'),
      render: (row) => (
        <span className="font-medium text-accent tabular-nums">{row.ref}</span>
      ),
    },
    {
      key: 'kind',
    ...
    },
    {
      ...
      align: 'right',
      render: (row) => (
        <span className="text-gray-900 dark:text-gray-100 tabular-nums">{formatCurrency(row.total, row.currency)}</span>
      ),
    },
    {
      key: 'date',
          ...
          <form onSubmit={handlePaymentSubmit} className="space-y-4">
            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg mb-4">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-gray-900 dark:text-gray-100 tabular-nums">{formatCurrency(Number(selectedOrder.totalAmount || 0))}</span>
              </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-ring focus-visible:border-transparent"
              />
            </div>
