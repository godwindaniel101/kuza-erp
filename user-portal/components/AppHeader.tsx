...
export default function AppHeader({ title = 'dashboard', subtitle }: AppHeaderProps) {
  ...
  // sidebar uses. This INCLUDES the tenant's own vertical (Restaurant / Inventory
  // / Storefront) so there is always a way back to it from a sub-app (Invoicing,
  // Accounting, …). Other verticals never appear — availableCoarseApps already
  // gates by businessType, so only the tenant's active vertical surfaces here.
      ...
      <div className="flex h-full items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <nav aria-label="Breadcrumb" className="flex min-w-0 items-center gap-1.5 text-[13px]">
            ...
            <span className="truncate font-display font-semibold tracking-tight text-gray-900 dark:text-gray-100">{t(title) || title}</span>
          </nav>
        </div>
