...
export default function AdminTenantDetailPage() {
  ...
  const pendingRequests: AdminAccessRequest[] = useMemo(() => {
    const list = tenant?.pendingAccessRequests ?? tenant?.accessRequests ?? [];
    return list.filter((r) => !r.status || r.status.toUpperCase() === 'PENDING');
  }, [tenant]);
  ...
  if (loadError || !tenant) {
    return (
      <div className="w-full max-w-3xl space-y-5">
        <PageHeader
          title="Tenant"
          breadcrumbs={[{ label: 'Tenants', href: '/tenants' }, { label: 'Detail' }]}
        />
        <EmptyState
          icon="bx-buildings"
          ...
          actions={
            <>
              <Button size="sm" variant="secondary" href="/tenants">
                Back to tenants
              </Button>
  ...
  const status = tenantSubscriptionStatus(tenant);
  const statusLabel = status.charAt(0).toUpperCase() + status.slice(1);
  const trialEnds = formatDate(tenant.trialEndsAt);
  ...
  return (
    <div className="w-full max-w-4xl space-y-5">
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Tenants', href: '/tenants' }, { label: tenant.name || 'Detail' }]}
        actions={<StatusBadge variant={subVariant(status)} label={statusLabel} size="sm" />}
      />
        ...
        <dl className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <dd className="mt-1 flex flex-col gap-1">
              <StatusBadge variant={subVariant(status)} label={statusLabel} size="sm" />
              {trialEnds && (
                <span className="text-xs text-gray-500 dark:text-gray-400">Trial ends {trialEnds}</span>
              )}
            </dd>
          </div>
        </dl>
