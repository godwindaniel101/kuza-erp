...
export interface AdminTenant {
  ...
  status?: string | null;
  isActive?: boolean;
  /** Subscription lifecycle (trialing | active | expired | …) — read via `tenantSubscriptionStatus()`. */
  subscriptionStatus?: string | null;
  /** When the trial ends (subscriptionStatus === 'trialing'). */
...
export interface AdminTenantDetail extends AdminTenant {
  apps?: AdminAppState[];
  /** Pending requests may arrive as `accessRequests` or `pendingAccessRequests`. */
  accessRequests?: AdminAccessRequest[];
  pendingAccessRequests?: AdminAccessRequest[];
}
...
/**
 * Subscription lifecycle string ('trialing' | 'active' | 'expired' | …),
 * best-effort across payload shapes. Falls back to the generic status.
 */
export function tenantSubscriptionStatus(t: AdminTenant): string {
  if (typeof t.subscriptionStatus === 'string' && t.subscriptionStatus) {
    return t.subscriptionStatus.toLowerCase();
  }
  return tenantStatus(t).toLowerCase();
}

/** Number of enabled apps for a tenant, best-effort across shapes. */
export function tenantAppsCount(t: AdminTenant): number {
  if (typeof t.enabledAppCount === 'number') return t.enabledAppCount;
