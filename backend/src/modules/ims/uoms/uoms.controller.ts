...
export class UomsController {
  ...
  async create(@Body() createDto: CreateUomDto, @I18n() i18n: I18nContext) {
    const uom = await this.uomsService.create(createDto);
    return {
      success: true,
      data: uom,
  ...
  async findAll() {
    const uoms = await this.uomsService.findAll();
    return {
      success: true,
      data: uoms,
    };
  ...
  async findOne(@Param("id") id: string) {
    const uom = await this.uomsService.findOne(id);
    return {
      success: true,
      data: uom,
    };
  ...
  async update(
    ...
    const uom = await this.uomsService.update(id, updateDto);
    return {
      success: true,
      data: uom,
  ...
  async remove(@Param("id") id: string, @I18n() i18n: I18nContext) {
    await this.uomsService.remove(id);
    return {
      success: true,
  ...
  async getConvertibleUoms(@Param("id") id: string) {
    const result = await this.uomsService.getConvertibleUoms(id);
    return {
      success: true,
      data: result,
    };
