...
/**
 * Resolves the tenant for the UNAUTHENTICATED public storefront endpoint.
 * A copy of MenuSiteTenantGuard, reading the landlord `storefront_slug_routes`
 * table instead of `menu_slug_routes`.
 *
 ...
 *   landlord `storefront_slug_routes` table and attaches
 *   `request.tenant = { id, schemaName }` — exactly what TenantGuard would have
 *   attached for an authenticated request.
 * - The global TenantTransactionInterceptor then sees
 *   `request.tenant.schemaName`, pins one connection for the request inside a
 *   transaction and issues `SET LOCAL search_path TO "<schema>", public`. Every
 *   tenant-connection repository call in the handler therefore reads the
 *   correct tenant schema, with no auth and no cookies.
 *
 * The schemaName comes from our own landlord table (never user input), but
 * because it is interpolated into `SET LOCAL search_path` we still validate its
 * shape defensively before attaching it.
 */
@Injectable()
export class StoreTenantGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    request.storefrontSlugRoute = route;
