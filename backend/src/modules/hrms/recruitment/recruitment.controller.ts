...
export class RecruitmentController {
  ...
  async createPosting(
    ...
    const posting = await this.recruitmentService.createJobPosting(createDto);
    return {
      success: true,
      data: posting,
  ...
  async findAllPostings(@Request() req, @Query("status") status?: string) {
    const postings = await this.recruitmentService.findAllJobPostings(status);
    return {
      success: true,
      data: postings,
    };
  ...
  async findOnePosting(@Request() req, @Param("id") id: string) {
    const posting = await this.recruitmentService.findOneJobPosting(id);
    return {
      success: true,
      data: posting,
    };
  ...
  async updatePosting(
    ...
    const posting = await this.recruitmentService.updateJobPosting(
      id,
      updateDto,
    );
    return {
  ...
  async removePosting(
    ...
    await this.recruitmentService.removeJobPosting(id);
    return {
      success: true,
  ...
  async createApplication(
    ...
    const application = await this.recruitmentService.createApplication(
      createDto.jobPostingId,
      createDto,
    );
    return {
      success: true,
  ...
  async findAllApplications(@Query("jobPostingId") jobPostingId?: string) {
    const applications =
      await this.recruitmentService.findAllApplications(jobPostingId);
    return {
      success: true,
      data: applications,
    };
