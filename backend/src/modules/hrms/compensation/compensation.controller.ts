...
export class CompensationController {
  ...
  async createStructure(
    ...
    const structure = await this.compensationService.createStructure(createDto);
    return {
      success: true,
      data: structure,
  ...
  async findAllStructures(@Query("isActive") isActive?: string) {
  ...
  async findOneStructure(@Param("id") id: string) {
    const structure = await this.compensationService.findOneStructure(id);
    return {
      success: true,
      data: structure,
    };
  ...
  async updateStructure(
    ...
    const structure = await this.compensationService.updateStructure(
      id,
      updateDto,
    );
    return {
  ...
  async removeStructure(@Param("id") id: string, @I18n() i18n: I18nContext) {
    await this.compensationService.removeStructure(id);
    return {
      success: true,
  ...
  async createEmployeeSalary(
    ...
    const salary =
      await this.compensationService.createEmployeeSalary(createDto);
    return {
      success: true,
      data: salary,
      message: i18n.t("common.created"),
  ...
  async findAllEmployeeSalaries(@Query("employeeId") employeeId?: string) {
    const salaries =
      await this.compensationService.findAllEmployeeSalaries(employeeId);
    return {
      success: true,
      data: salaries,
    };
