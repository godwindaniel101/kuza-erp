...
export class LearningController {
  ...
  async createCourse(
    ...
    const course = await this.learningService.createCourse(createDto);
    return {
      success: true,
      data: course,
  ...
  async findAllCourses(@Query("status") status?: string) {
    const courses = await this.learningService.findAllCourses(status);
    return {
      success: true,
      data: courses,
    };
  ...
  async findOneCourse(@Param("id") id: string) {
    const course = await this.learningService.findOneCourse(id);
    return {
      success: true,
      data: course,
    };
  ...
  async updateCourse(
    ...
    const course = await this.learningService.updateCourse(id, updateDto);
    return {
      success: true,
      data: course,
  ...
  async removeCourse(@Param("id") id: string, @I18n() i18n: I18nContext) {
    await this.learningService.removeCourse(id);
    return {
      success: true,
  ...
  async findAllEnrollments(
    ...
    const enrollments = await this.learningService.findAllEnrollments(
      employeeId,
      courseId,
    );
    return {
